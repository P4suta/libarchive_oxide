...
impl Lz4Decoder {
    ...
    fn parse_header(&mut self) -> Result<bool, ArchiveError> {
        ...
        let max_block = match block_code {
            4 => 64 * 1024,
            5 => 256 * 1024,
            6 => 1024 * 1024,
            7 => MAX_BLOCK,
        ...
        if let Some(limit) = self.limits.codec_memory() {
            let dictionary = usize::from(flags & FLG_INDEPENDENT == 0) * WINDOW;
            let required = max_block
                .checked_mul(2)
                .and_then(|bytes| bytes.checked_add(dictionary))
                .and_then(|bytes| bytes.checked_add(MAX_HEADER + 8))
                .ok_or_else(|| {
                    ArchiveError::new(ErrorKind::Limit)
                        .with_format("lz4")
                        .with_context("LZ4 codec-memory accounting overflow")
                })?;
            if required > limit {
                return Err(ArchiveError::new(ErrorKind::Limit)
                    .with_format("lz4")
                    .with_context("LZ4 frame exceeds the configured codec-memory budget"));
            }
        }
    ...
    fn finish_frame(&mut self, record_length: usize) -> Result<(), ArchiveError> {
        if let Some(expected) = self
            .content_size
            .filter(|expected| self.content_length != *expected)
        {
    ...
    fn parse_block(&mut self) -> Result<bool, ArchiveError> {
        ...
        if self
            .content_size
            .is_some_and(|expected| self.content_length > expected)
        {
    ...
    fn pending_limit(&self) -> usize {
        match self.state {
            State::Header => MAX_HEADER,
            State::Blocks => self.max_block.saturating_add(8),
        }
    }

    fn stage_input(&mut self, input: &[u8]) -> usize {
        let available = self.pending_limit().saturating_sub(self.pending.len());
        let accepted = available.min(input.len());
        self.pending.extend_from_slice(&input[..accepted]);
        accepted
    }
}
...
impl Codec for Lz4Decoder {
    fn process(
        ...
        if output.is_empty() {
            let consumed = self.stage_input(input);
        ...
        loop {
            ...
            // Parse before staging more bytes, then recompute the bound after
            // every Header -> Blocks or frame -> Header transition. A single
            // supplied slice can therefore cross states without either reading
            // ahead unnecessarily or returning NeedInput while bytes remain.
            ...
            let accepted = self.stage_input(&input[consumed..]);
            consumed += accepted;
            if accepted == 0 {
                break;
            }
        ...
        let status = if consumed != input.len() {
            return Err(ArchiveError::new(ErrorKind::Protocol)
                .with_format("lz4")
                .with_context("LZ4 decoder could not accept the supplied input"));
...
pub fn encode_frame(input: &[u8]) -> Result<Vec<u8>, ArchiveError> {
    const ENCODER_FLAGS: u8 = FLG_VERSION_ONE
        | FLG_INDEPENDENT
        | FLG_BLOCK_CHECKSUM
        | FLG_CONTENT_SIZE
        | FLG_CONTENT_CHECKSUM;
    const ENCODER_BLOCK_DESCRIPTOR: u8 = 4 << 4;
    const ENCODER_BLOCK: usize = 64 * 1024;

    let content_size = u64::try_from(input.len()).map_err(|_| {
        ArchiveError::new(ErrorKind::Limit)
            .with_format("lz4")
            .with_context("LZ4 input length exceeds the format address space")
    })?;
    let mut output = Vec::new();
    output.extend_from_slice(MAGIC);
    output.push(ENCODER_FLAGS);
    output.push(ENCODER_BLOCK_DESCRIPTOR);
    output.extend_from_slice(&content_size.to_le_bytes());

    let mut header_hasher = XxHash32::with_seed(0);
    header_hasher.write(&output[MAGIC.len()..]);
    output.push(header_hasher.finish().to_le_bytes()[1]);

    for chunk in input.chunks(ENCODER_BLOCK) {
        let compressed = lz4_flex::block::compress(chunk);
        let (record, stored) = if compressed.len() < chunk.len() {
            (compressed.len(), compressed.as_slice())
        } else {
            (chunk.len(), chunk)
        };
        let record = u32::try_from(record).map_err(|_| {
            ArchiveError::new(ErrorKind::Limit)
                .with_format("lz4")
                .with_context("LZ4 block length exceeds the format address space")
        })?;
        let record = if stored.len() == chunk.len() {
            record | BLOCK_UNCOMPRESSED
        } else {
            record
        };
        ...
        block_hasher.write(stored);
        output.extend_from_slice(&Lz4Decoder::digest32(&block_hasher).to_le_bytes());
    }

    output.extend_from_slice(&0_u32.to_le_bytes());
    let mut content_hasher = XxHash32::with_seed(0);
    content_hasher.write(input);
    output.extend_from_slice(&Lz4Decoder::digest32(&content_hasher).to_le_bytes());
    Ok(output)
}
