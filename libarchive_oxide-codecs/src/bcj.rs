...
/// Incremental BCJ decoder driving its branch transform with a small carry buffer so
/// that instruction windows straddling input chunks are handled correctly.
pub struct BcjDecoder {
    filter: BcjFilter,
...
impl BcjDecoder {
    ...
    #[must_use]
    pub fn new(kind: BranchKind, start: usize) -> Self {
        Self {
...
mod tests {
    ...
    fn portable_decode(
        ...
        drive_codec(BcjDecoder::new(kind, 0), encoded, in_chunk, out_chunk)
    }
    ...
    fn every_branch_kind_round_trips_against_lzma_rust2() {
        ...
        for kind in [
            ...
            // The portable codec decodes byte-identically regardless of chunking.
            for (out_chunk, in_chunk) in [(1, 1), (3, 7), (64, 5), (4096, 64), (200_000, 200_000)] {
                assert_eq!(
                    portable_decode(kind, &encoded, out_chunk, in_chunk),
                    data,
                    "portable decode mismatch for {kind:?} at out={out_chunk} in={in_chunk}"
                );
            }
        }
    }
    ...
    fn tiny_output_matches_bulk_output() {
        ...
        for len in [0usize, 1, 4, 5, 6, 8, 4095, 4096, 4097] {
            ...
            let bulk = portable_decode(
                BranchKind::X86,
                &encoded,
                usize::max(len, 1),
                usize::max(len, 1),
            );
            let single = portable_decode(BranchKind::X86, &encoded, 1, 1);
