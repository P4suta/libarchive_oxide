...
use arca_core::format::iso9660::{IsoReader, IsoWriter};
use arca_core::{EntryData, EntryReader, EntryWriter};
...
fn valid_image() -> Vec<u8> {
    ...
    let mut m = arca_core::EntryMeta::new(
        arca_core::EntryKind::Dir,
        std::borrow::Cow::Borrowed(&b"d"[..]),
    );
    m.size = 0;
    w.start_entry(&m).unwrap().close().unwrap();
    let mut f = arca_core::EntryMeta::new(
        arca_core::EntryKind::File,
