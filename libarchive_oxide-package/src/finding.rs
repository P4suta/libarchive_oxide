...
pub enum PackageFindingCode {
    ...
    ContainerUnreadable,
    /// The outer container did not match the selected package ecosystem.
    ...
    /// Package metadata was syntactically invalid.
    InvalidPackageMetadata,
    /// Package metadata exceeded the configured allocation budget.
    MetadataTooLarge,
    /// A signature member name or position violated the package profile.
    InvalidSignatureMember,
    /// A declared package digest or size did not match the entry bytes.
    IntegrityMismatch,
    /// A manifest or integrity record was malformed.
    InvalidIntegrityMetadata,
    /// A required entry had no integrity record.
    MissingIntegrityRecord,
    /// An integrity record requested an algorithm this verifier does not use.
    UnsupportedIntegrityAlgorithm,
    /// An integrity record covers a byte stream this verifier cannot observe.
    UnsupportedIntegrityScope,
    /// Entry bytes could not be read while calculating integrity.
    IntegrityReadFailure,
    /// Integrity calculation exceeded the caller's configured byte budget.
    IntegrityResourceLimit,
    /// A CMS signature container or JAR signature file was malformed.
    InvalidSignatureMetadata,
    /// An explicitly supplied detached signature sidecar was malformed.
    InvalidSignatureSidecar,
    /// A detached signature sidecar did not bind to the supplied package.
    SignatureSidecarMismatch,
    /// An optional detached signature sidecar was not supplied.
    SignatureSidecarNotProvided,
    /// A signature, signed digest, or signed manifest did not match its input.
    SignatureMismatch,
    /// A signature names a verification key the offline caller did not supply.
    MissingVerificationKey,
    /// A signature requested a cryptographic algorithm this verifier cannot use.
    UnsupportedSignatureAlgorithm,
    /// Signature parsing exceeded a configured byte or object-count budget.
...
impl PackageFindingCode {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ContainerFormatMismatch => "container-format-mismatch",
            ...
            Self::InvalidPackageMetadata => "invalid-package-metadata",
            Self::MetadataTooLarge => "metadata-too-large",
            Self::InvalidSignatureMember => "invalid-signature-member",
            Self::IntegrityMismatch => "integrity-mismatch",
            Self::InvalidIntegrityMetadata => "invalid-integrity-metadata",
            Self::MissingIntegrityRecord => "missing-integrity-record",
            Self::UnsupportedIntegrityAlgorithm => "unsupported-integrity-algorithm",
            Self::UnsupportedIntegrityScope => "unsupported-integrity-scope",
            Self::IntegrityReadFailure => "integrity-read-failure",
            Self::IntegrityResourceLimit => "integrity-resource-limit",
            Self::InvalidSignatureMetadata => "invalid-signature-metadata",
            Self::InvalidSignatureSidecar => "invalid-signature-sidecar",
            Self::SignatureSidecarMismatch => "signature-sidecar-mismatch",
            Self::SignatureSidecarNotProvided => "signature-sidecar-not-provided",
            Self::SignatureMismatch => "signature-mismatch",
            Self::MissingVerificationKey => "missing-verification-key",
            Self::UnsupportedSignatureAlgorithm => "unsupported-signature-algorithm",
            Self::SignatureResourceLimit => "signature-resource-limit",
        }
    }
    ...
    pub const fn default_severity(self) -> Severity {
        match self {
            ...
            | Self::CompressorMismatch
            | Self::UnsupportedIntegrityAlgorithm
            | Self::UnsupportedIntegrityScope
            | Self::MissingVerificationKey
            | Self::UnsupportedSignatureAlgorithm => Severity::Warning,
            Self::UnsignedPackage
            | Self::SigningSchemeDetected
            | Self::SignatureSidecarNotProvided => Severity::Info,
            _ => Severity::Error,
        }
    }
}
...
impl PackageFinding {
    ...
    /// decode, mirroring [`libarchive_oxide::advanced::ProviderCapability::Disabled`].
    #[must_use]
