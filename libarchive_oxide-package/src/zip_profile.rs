...
const NUGET_SIGNATURE: &[u8] = b".signature.p7s";
...
impl ZipPackageValidator {
    ...
    pub fn validate<R: Read + Seek>(&self, mut reader: R) -> ZipPackageValidation {
        ...
        let mut profile_satisfied = match profile {
        ...
        if profile == ZipPackageProfile::NuGet
            ...
            && !crate::nuget_signature::inspect_nuspec(&mut reader, self.limits, &mut findings)
        {
...
#[allow(
    ...
    reason = "NuGet root-member, signature-placement, and collision checks share one pass"
)]
fn check_nuget(
    label: &'static str,
    entries: &[ZipEntry],
    ...
    let mut ok = true;
    let mut collision_keys = BTreeMap::<Vec<u8>, &[u8]>::new();
    for entry in entries {
        let key = nuget_path_key(&entry.name);
        if let Some(previous) = collision_keys.insert(key, &entry.name)
            && previous != entry.name
        {
            findings.push(PackageFinding::new(
                label,
                ...
                format!(
                    "member path collides case-insensitively with {}",
    ...
    let signature_like = entries
        .iter()
        .filter(|entry| entry.name.eq_ignore_ascii_case(NUGET_SIGNATURE))
        .collect::<Vec<_>>();
    if signature_like.len() > 1 {
        findings.push(PackageFinding::new(
            label,
            None,
            PackageFindingCode::InvalidSignatureMember,
            "archive has more than one case-insensitive .signature.p7s member",
        ));
        ok = false;
    }
    if let Some(signature) = signature_like.first() {
        if signature.name != NUGET_SIGNATURE {
            findings.push(PackageFinding::new(
                label,
                Some(signature.name.clone()),
                PackageFindingCode::InvalidSignatureMember,
                "NuGet signature member name must be exactly .signature.p7s",
            ));
            ok = false;
        }
        if signature.method != METHOD_STORE {
            findings.push(PackageFinding::new(
                ...
                Some(signature.name.clone()),
                PackageFindingCode::InvalidSignatureMember,
                "NuGet signature member must use Store compression",
            ));
            ok = false;
        }
        if signature.flags & ZIP_FLAG_UTF8 != 0 {
            findings.push(PackageFinding::new(
                label,
                Some(signature.name.clone()),
                PackageFindingCode::InvalidSignatureMember,
                "NuGet signature member must use the ZIP non-UTF-8 filename encoding",
            ));
            ok = false;
        }
        if entries
            .last()
            .is_none_or(|entry| entry.name != NUGET_SIGNATURE)
            || entries
                .iter()
                .any(|entry| entry.local_offset > signature.local_offset)
        {
            findings.push(PackageFinding::new(
                label,
                Some(signature.name.clone()),
                PackageFindingCode::InvalidSignatureMember,
                "NuGet signature member must be the final local and central-directory entry",
            ));
            ok = false;
        }
    }
...
fn nuget_path_key(name: &[u8]) -> Vec<u8> {
    match std::str::from_utf8(name) {
        Ok(name) => name
            ...
            .collect::<String>()
            .into_bytes(),
        Err(_) => name.iter().map(u8::to_ascii_lowercase).collect(),
    }
}
