...
/// APK Signature Scheme v3.1 block id.
...
impl AppSignatureReport {
    ...
    /// Whether any APK v1 signature metadata was present, including an
    /// incomplete `.SF` or signature-block pair.
    pub(crate) const fn apk_v1_metadata(self) -> bool {
        self.apk_v1_metadata
    }
    ...
    /// Whether the APK Signature Scheme v3.1 block id was present.
    #[must_use]
    pub const fn apk_v3_1(self) -> bool {
        self.apk_v31
    }
...
impl AppPackageValidator {
    ...
    #[must_use]
    pub const fn android_apk() -> Self {
        Self::new(AppPackageProfile::AndroidApk)
    }
    ...
    fn check_apk<R: Read + Seek>(
        ...
        let (apk_v1, apk_v1_metadata) = detect_apk_v1(entries);
...
fn scan_apk_id_value_pairs(region: &[u8], signatures: &mut AppSignatureReport) {
    ...
    while cursor + 12 <= region.len() {
        ...
        match id {
            ...
            APK_V31_BLOCK_ID => signatures.apk_v31 = true,
            _ => {},
        }
        cursor = next;
    }
...
fn detect_apk_v1(entries: &[ZipEntry]) -> (bool, bool) {
    let signature_files: BTreeSet<Vec<u8>> = entries
        .iter()
        .filter_map(|entry| apk_v1_base(&entry.name, b".SF"))
        .collect();
    let signature_blocks: BTreeSet<Vec<u8>> = entries
        .iter()
        .flat_map(|entry| {
            [b".RSA".as_slice(), b".DSA", b".EC"]
                .into_iter()
                .filter_map(|suffix| apk_v1_base(&entry.name, suffix))
        })
        .collect();
    let complete_pair = signature_blocks
        .iter()
        .any(|base| signature_files.contains(base));
    let metadata_present = !signature_files.is_empty() || !signature_blocks.is_empty();
    (complete_pair, metadata_present)
}

fn apk_v1_base(name: &[u8], suffix: &[u8]) -> Option<Vec<u8>> {
    let rest = name.strip_prefix(b"META-INF/")?;
    if rest.is_empty() || rest.contains(&b'/') || !ascii_ends_with(rest, suffix) {
        return None;
    }
    let stem_length = rest.len().checked_sub(suffix.len())?;
    let stem = rest.get(..stem_length)?;
    (!stem.is_empty()).then(|| stem.iter().map(u8::to_ascii_uppercase).collect())
}
...
fn describe_apk(signatures: AppSignatureReport) -> String {
    ...
    if signatures.apk_v31 {
        schemes.push("v3.1");
    }
    if schemes.is_empty() && signatures.apk_signing_block {
        return "APK Signing Block present with no recognized v2/v3/v3.1 id".to_string();
    }
