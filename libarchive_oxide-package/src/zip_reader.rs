...
fn zip64_sizes(
    ...
    Ok((uncompressed_size, compressed_size, local_offset))
}
...
pub(crate) fn check_common_structure(
    ...
    for entry in entries {
        let Ok(path) = ArchivePath::try_from_encoded(entry.name.clone(), entry.encoding) else {
            findings.push(PackageFinding::new(
                label,
                Some(entry.name.clone()),
                PackageFindingCode::UnsafeEntryPath,
                "member name is invalid for its declared path encoding",
            ));
            continue;
        };
