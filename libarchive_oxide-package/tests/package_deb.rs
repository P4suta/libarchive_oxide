...
use libarchive_oxide_package::{DebValidator, PackageFindingCode};
...
/// Data filters whose encoders are available in the selected package features.
fn enabled_data_filters() -> Vec<Option<FilterId>> {
...
fn validate(bytes: &[u8]) -> libarchive_oxide_package::DebValidation {
...
fn well_formed_deb_is_valid_across_data_filters() {
    for filter in enabled_data_filters() {
...
fn unsupported_compression_reports_capability_finding() {
    ...
    let disabled_zstd = vec![0x28, 0xB5, 0x2F, 0xFD, 0, 0, 0, 0];
    let deb = build_ar(&[
        (b"debian-binary", b"2.0\n".to_vec()),
        (
            b"control.tar.gz",
