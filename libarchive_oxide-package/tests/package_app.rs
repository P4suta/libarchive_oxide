...
use libarchive_oxide_package::{
    AppPackageProfile, AppPackageValidation, AppPackageValidator, PackageFindingCode,
    PackageVerifier, VerificationDimension,
};
...
fn apk_v1_signed_reports_scheme_and_is_valid() {
    ...
    let result = validate(AppPackageProfile::AndroidApk, &bytes);
...
fn apk_v2_signing_block_is_detected() {
    ...
    let report = PackageVerifier::default().app(AppPackageProfile::AndroidApk, Cursor::new(bytes));
    assert_eq!(
        report.signature_validity(),
        VerificationDimension::Invalid,
        "the synthetic detection-only block is not a valid v2 signer"
    );
...
fn apk_v1_requires_both_sf_and_signature_file() {
    ...
    let bytes = build_real_zip(&[
        ...
        (b"META-INF/CERT.SF", b"Signature-Version: 1.0\n"),
    ]);
    let result = validate(AppPackageProfile::AndroidApk, &bytes);
    assert!(!result.signatures().apk_v1());
    assert!(result.has_code(PackageFindingCode::UnsignedPackage));
    ...
    assert_eq!(
        report.signature_validity(),
        VerificationDimension::Invalid,
        "partial v1 metadata must not be accepted as a clean unsigned APK"
    );
    assert!(
        report
            .findings()
...
fn decompression_bomb_is_refused_by_budget() {
    ...
    let validator =
        AppPackageValidator::android_apk().with_limits(Limits::safe().with_decoded_total(Some(64)));
...
fn apk_signing_block_scan_is_bounded_by_metadata_budget() {
    ...
    let validator = AppPackageValidator::android_apk()
        .with_limits(Limits::safe().with_metadata_bytes(Some(256)));
    let result = validator.validate(Cursor::new(bytes));
