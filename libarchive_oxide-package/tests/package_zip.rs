...
use libarchive_oxide_package::{PackageFindingCode, ZipPackageProfile, ZipPackageValidator};
...
fn build_zip(entries: &[RawEntry]) -> Vec<u8> {
    ...
    for entry in entries {
        ...
        let crc32 = crc32fast::hash(&entry.body);
...
fn validate(
    ...
    bytes: &[u8],
) -> libarchive_oxide_package::ZipPackageValidation {
...
fn nuget_with_content_types_and_single_nuspec_is_valid() {
    let bytes = build_zip(&[
        ...
        RawEntry::stored(
            b"demo.nuspec",
            b"<package><metadata><id>demo</id><version>1.0.0</version>\
              <authors>Example</authors><description>fixture</description>\
              </metadata></package>",
        ),
...
#[test]
fn nuget_case_colliding_paths_are_rejected() {
    let bytes = build_zip(&[
        RawEntry::stored(b"[Content_Types].xml", b"<Types/>"),
        RawEntry::stored(
            b"demo.nuspec",
        ...
        RawEntry::stored(b"lib/net8.0/Demo.dll", b"one"),
        RawEntry::stored(b"LIB/NET8.0/demo.DLL", b"two"),
    ]);
    let result = validate(ZipPackageProfile::NuGet, &bytes);
    assert!(!result.profile_valid());
