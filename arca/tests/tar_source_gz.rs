//! End-to-end incremental pipeline: the byte-axis filter [`arca_filter::AnyDecoder`] (gzip) feeding
...
use arca_core::{EntryData, EntryKind, EntryReader, EntrySource, SourceEvent, Status, Transform};
...
fn pipeline(gz: &[u8], chunk: usize) -> Vec<Ent> {
    let mut dec = arca_filter::decoder(FilterId::Gzip).unwrap();
